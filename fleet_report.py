...
def car_wear(car: dict) -> float:
    """Return the wear percentage for a single car, handling missing readings."""
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a summary dict with fleet count, cars due, and average wear."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
...
def print_report(fleet: list[dict]) -> None:
    """Print the nightly fleet-health report to stdout and flush to the log file."""
